...
add_journey_request_driver: async function (ucid, user_type, no_passengers, arrival_time){
    //autogenerated ID
    const data={
        ucid: ucid,
        user_type: user_type,
...
add_journey_request_passenger: async function (ucid, user_type,arrival_time){
    //autogenerated ID
    const data={
        ucid: ucid,
        user_type: user_type,
...
{ //pass the id of document -> id of specific organization
    ...
    snapshot.forEach(doc => {
       const res = db.collection('journey_request').doc(doc.id).delete();
    });
},
...
get_carpool_trip: async function(user_id){
    const journye_requests = db.collection('carpool_trip');
    var carpool_trip;
    var found=0;
    const snapshot = await journye_requests.where('no_of_passenger', '!=', null).get();
    snapshot.forEach(doc => {
        var arrayLength = doc.data().passengers.length;
        for (var i = 0; i < arrayLength; i++) {
           if(user_id==passengers[i]){
                carpool_trip= doc.data();
                found=1;
           }
        }
    })
...
delete_carpool_trip: function(carpool_trip_id){ //pass the id of document -> id of specific organization
    (async function(){
        const res = await db.collection('carpool_trip').doc(carpool_trip_id).delete();
    })()
}
};
